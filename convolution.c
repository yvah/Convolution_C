                  ...
                  int kernel_order)
{
    ...
    int16_t *kernel_pointer = (int16_t *)***kernels;
    ...
    #pragma omp parallel for (m, w, h, c, x, y, sum4, temp) collapse(3)
    for (m = 0; m < nkernels; m++)
    {
        mo_index = m * mo_mult;
        m_index = m * m_mult;
                ...
                for (c = 0; c < nchannels; c++)
                {
                    ...
                    if (kernel_order = 1)
                    {
                        for (x = 0; x < kernel_order; x++)
                        {
                            ...
                            sum4 = _mm_add_ps(sum4, _mm_mul_ps(_mm_set_ps(0, 0, image[w + x][h][c], 0), _mm_cvtepi32_ps(_mm_set1_epi16(kernel_pointer[x_index]))));
                        }
                    }
                    else if (kernel_order = 3)
                    {
                        ...
                        {
                            x_index = x * kernel_order + c_index;
                            sum4 = _mm_add_ps(sum4, _mm_mul_ps(_mm_set_ps(image[w + x][h + 2][c], image[w + x][h + 1][c], image[w + x][h][c], 0), _mm_cvtepi32_ps(_mm_set1_epi16(kernel_pointer[x_index]))));
                        }
                    }
                    else if (kernel_order = 5)
                    {
                        for (x = 0; x < kernel_order; x += 5)
                        {
                            x_index = x * kernel_order + c_index;
                            sum4 = _mm_add_ps(sum4, _mm_mul_ps(_mm_set_ps(image[w + x][h + 4][c], image[w + x][h + 3][c], image[w + x][h + 2][c], image[w + x][h + 1][c]), _mm_cvtepi32_ps(_mm_set1_epi16(kernel_pointer[x_index]))));
                            sum4 = _mm_add_ps(sum4, _mm_mul_ps(_mm_set_ps(0, 0, 0, image[w + x][h][c]), _mm_cvtepi32_ps(_mm_set1_epi16(kernel_pointer[x_index + 1]))));
                        }
                    }
                    else if (kernel_order = 7)
                    {
                        for (x = 0; x < kernel_order; x += 7)
                        {
                            sum4 = _mm_add_ps(sum4, _mm_mul_ps(_mm_set_ps(image[w + x][h + 6][c], image[w + x][h + 5][c], image[w + x][h + 4][c], image[w + x][h + 3][c]), _mm_cvtepi32_ps(_mm_set1_epi16(kernel_pointer[x_index]))));
                            sum4 = _mm_add_ps(sum4, _mm_mul_ps(_mm_set_ps(0, image[w + x][h + 2][c], image[w + x][h + 1][c], image[w + x][h][c]), _mm_cvtepi32_ps(_mm_set1_epi16(kernel_pointer[x_index + 3]))));                          
                        }
                    }
